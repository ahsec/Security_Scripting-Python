...
# thread.allocate_Lock - Locks a thread until it finishes it's tasks, then it
# releases it's resources and allows the program to terminate gracefully
  ...
  def run(self):
    try:
      ...
      while True:
        ...
        ftp.login()
        print "======== Retrieving FTP DIR info from:  %s =======" %(site)
        ftp.retrlines('LIST')
        print "======== FTP List retrieved ========"
        ftp.quit()
        self.queue.task_done()
    except Exception as e:
      print 'Exception: %s' %(e)
    finally:
      # No matter what (Exception raised or not, the Lock gets released and
      # all the resources asigned to it too.
      gLock.release()
      self.queue.task_done()
