...
def show_progress(num_blks, blk_size, tot_size):
  '''
  This function prints the progress of the transmission.
  It is being called from the urllib.urlretrieve function
  '''
...
def get_file(url):
  ...
  if code != 200:
    ...
    name = parts[-1]
    # Retrieves the file from the URL using the function "show_progress"
    # to show the progress, duh!
...
def usage():
  print """Usage: Retrieve_File_progress.py URL
        This script will retrieve the file located in the URL and it will
        show the progress of such transmission URL must be in the form
        http://any.com/file.pdf"""
