...
# This program will download all the HTML files it finds in a specific URL. It will follow "num" number of links inside the original URL
##########################################
...
def get_code_links_html(url):
  # This function will return the html code and all the links (href) found in it
...
def web_spider(url, num_links):
  ...
  # While we haven't visited enough number of links (specified by command line) the program will call itself
  while len(list_final_data) <= num_links:
    # Get HTML code and links from URL
    (code, html, linksl) = get_code_links_html(url)
    # Add results to the "list_final_data" list. Which is the main results list
    ...
    for element in linksl :
      ...
      # Will add each element to the beginning of the url_to_visit and then will visit 
      # each site from that list (until reach the maximum number of links to visit)
