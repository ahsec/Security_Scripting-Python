...
def usage():
  print """paramiko_ssh_bruteforce.py Host dictionary_file
        This script will try to bruteforce a SSH service using a dcitionary
        and the root userid"""

def get_passwd_list(dictionary):
  '''Function to read the contents of a file and return it as a list
  (dictionary file).
  '''
...
def bruteforce(host, dictionary):
  ...
  for passwd in passwd_list:
    try :
      ...
      print e
      # In any other case (timeout, server closed connection)
      # it will restart the client and continue the procedure
      client = paramiko.SSHClient()
      client.load_system_host_keys()
    finally:
