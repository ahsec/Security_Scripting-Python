...
def syn_scan(ip):
  ans, unans = sr(IP(dst = ip)/TCP(flags = 'S', dport = [21,22,23,80,56,443]),
                  timeout = 2)
  for i in range (0,len(ans)-1):
    print 'Port: %s - TCP flags: %s' %(ans[i][1][TCP].sport,
                                       ans[i][1][TCP].flags)
...
def usage():
  print """Usage: Scapy_SYN_scan.py IP_Address
        This script will perform a SYN scan of the ports [21,22,23,80,443] for
        the IP_Address host"""
